...
function openWorkoutDetail(weekIndex, dayIndex, dayPlan) {
  ...
  dayPlan.work.forEach((ex, exIndex) => {
    ...
    // v7.12 FIX #5: Add "Copy" column for manual weight copying
    table.innerHTML = `<thead><tr><th>Set</th><th>Weight</th><th>Reps</th><th>RPE</th><th>Action</th><th style="width:50px;">Copy</th></tr></thead><tbody></tbody>`;
    ...
    scheme.forEach((s, setIndex) => {
      ...
      row.innerHTML = `
        ...
        <td style="padding:6px; text-align:center;"><button class="secondary small" data-role="copy" style="padding:4px 8px; font-size:16px;" title="Copy to remaining sets">↓</button></td>
      `;
      ...
      // v7.12 FIX #5: Copy button - manually copy weight to remaining sets
      if (copyBtn && scheme[setIndex]?.tag === 'work') {
        copyBtn.addEventListener('click', () => {
          const currentWeight = wEl.value;
          const currentReps = repsEl.value;
          
          if (!currentWeight && !currentReps) return; // Nothing to copy
          
          // Copy to all subsequent work sets
          for (let j = setIndex + 1; j < scheme.length; j++) {
            if (scheme[j]?.tag !== 'work') continue;
            ...
            if (nextRow) {
              const nextWEl = nextRow.querySelector('[data-role="weight"]');
              const nextRepsEl = nextRow.querySelector('[data-role="reps"]');
              
              if (currentWeight && nextWEl) {
                nextWEl.value = currentWeight;
                updateRec(j, { weight: currentWeight });
              }
              
              if (currentReps && nextRepsEl) {
                nextRepsEl.value = currentReps;
                updateRec(j, { reps: currentReps });
              }
            }
          }
        ...
        // Hide copy button for warmup sets
