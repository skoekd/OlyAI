  ...
  sessions.forEach((s, si) => {
    ...
    if (programType === 'powerbuilding') {
      ...
      if (s.kind === 'accessory') {
        ...
        if (duration >= 90) {
          const pullEx = chooseVariation('pull_clean', profile, weekIndex, phase, 'cj_pull_extra');
          s.work = [...s.work,
            { name: pullEx.name, liftKey: pullEx.liftKey, sets: Math.round(3 * volFactor), reps: 3, pct: clamp(intensity + 0.08, 0.55, 0.92) },
    ...
    // HYPERTROPHY: Higher volume bodybuilding
      ...
      if (s.kind === 'accessory' && duration >= 75) {
        s.work = [
          ...s.work,
          { name: chooseHypertrophyExercise('upperPush', profile, weekIndex, 'hyp_acc_extra1').name, sets: hypSets, reps: 12, pct: 0, tag: 'hypertrophy' },
          { name: chooseHypertrophyExercise('shoulders', profile, weekIndex, 'hyp_acc_extra2').name, sets: 3, reps: 15, pct: 0, tag: 'hypertrophy' }
        ];
    ...
    const renderDayCard = (day, dayIndex, isAccessory = false) => {
      ...
      header.innerHTML = `
        <div class="day-header-left">
          ...
          <div class="mini-badge" style="background:${isAccessory ? badgeColor : ''}">${day.title}</div>
        </div>
...
function completeDay(weekIndex, dayIndex, dayPlan) {
  ...
  const session = {
    ...
    work: dayPlan.work.map((ex) => {
      ...
      let weightText = '';
      if (ex.pct && liftKey) {
        const base = getAdjustedWorkingMax(p, liftKey) || (p.maxes?.[liftKey] || 0);
