...
const SWAP_POOLS = {
  snatch: [
    ...
    { name: 'Power Snatch + OHS', liftKey: 'snatch' },
    { name: 'Hang Snatch (hip) + Snatch', liftKey: 'snatch' },
    { name: 'Snatch High Pull + Hang Snatch', liftKey: 'snatch' }
  ],
  cj: [
    ...
    { name: 'Clean Pull + Clean', liftKey: 'cj' },
    { name: 'Power Clean + Front Squat', liftKey: 'cj' },
      ...
      if (aEl) {
        ...
        aEl.addEventListener('change', () => {
          ...
          if (scheme[setIndex]?.tag === 'work') {
            // CRITICAL FIX: Update ALL subsequent work sets, even if they have values
            // This ensures checkmark/action changes propagate forward
            for (let j = setIndex + 1; j < scheme.length; j++) {
              if (scheme[j]?.tag !== 'work') continue;
